...
EXPERIMENT="expresults/veri_time_september_bb_branch-dynamic-frames*"
...
def parse_all():
    ...
    path_filter = included_paths()
    print(path_filter)
    openers = [(fn, opener) for fn, opener in openers if any(pf in fn for pf, fn2 in path_filter)]
    print([fn for fn, opener in openers])
...
class CdfPoint:
    def __init__(self, tex_label, pct_thresh, pile):
        self.tex_label = tex_label
        self.pct_thresh = pct_thresh

        # print([pile.cdist(idx) for idx in range(pile.N())])
        idx,t = next((idx,t) for idx,t in enumerate(pile.avg_times()) if pile.cdist(idx) >= pct_thresh)
        self.t = t
        self.idx = idx - 1
...
class CDFStuff:
    def __init__(self, pile):
        ...
        self.point90p = CdfPoint("90p", .9, pile)
        # self.point20s = CdfPoint("twentySec", 20, pile)

    def defs(self):
        defs = {}
        defs.update(self.point90p.defs)
...
def plot_all(groupname, pile):
    ...
    for point in (cdf.point90p,):
        ylogThr = complog(point.yThr)
        y0 = complog(0)
        ax.plot([0, point.tThr, point.tThr], [ylogThr, ylogThr, y0], color="gray")
        yOff = 0.80 if point==cdf.point90p else 0.99
        ax.text(point.tThr*1.01, ylogThr*yOff, va="top", s="%s%% faster than %ds" % (point.pct_text, point.t))
...
def percentiles(groupname, pile):
    ...
    for pct_thresh_percent in percentiles:
        ...
        try:
            idx,t = next((idx,t) for idx,t in enumerate(pile.avg_times()) if pile.cdist(idx) >= pct_thresh)
            output.append((groupname, pct_thresh_percent, t))
        except:
            pass
    output.append((groupname, 1, pile.avg_times()[-1]))
    with open("../veripapers/osdi2020/data/veritimecomp/dynamic-frames.data", "w") as fp:
        fp.write("% generated by tools/plot/plot-verification-time-comparison.py\n")
        fp.write("branch\tpercentile\ttime\n")
        for out_tuple in output:
            fp.write("\t".join([str(x) for x in out_tuple]))
            fp.write("\n")
