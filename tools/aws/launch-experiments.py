...
common_vars = [
    ...
    Variable("workload", "run_veri", [Value("wkc", "workload=ycsb/workloada-onefield.spec")]),
...
veri_suite = Suite(
    ...
    Variable("git_branch", "run_veri", [Value("osdi20-artifact-linear", "git_branch=osdi20-artifact-linear")]),
    #Variable("row_cache", "run_veri", [
    #    Value("%di" % bi, "env:ROW_CACHE_SIZE=%d cacheSize=%d" % (1<<bi, 0.7*(1<<30) - (1<<bi)*600)) for bi in [3, 12, 15, 18, 21]]),
    Variable("cache_size", "run_veri", [Value("cache200nodes", "cacheSizeInNodes=200")]),
...
#    *common_vars)
suite = ConcatSuite("infra-test-012", veri_suite)
