...
module BlockCache refines Transactable {
  ...
  requires s.Ready?
  {
    ...
    // question : this last clause isn't needed
  ...
  predicate WriteBackIndirectionTableReq(s: Variables, s': Variables, dop: DiskOp, vop: VOp)
  {
    ...
    // question: seems too strong but something is necessary
    //  to prevent persistent from using outstanding locations
    //  could put in cleanup instead
  ...
  predicate IvyEvict(s: Variables, s': Variables)
  {
  exists r: Reference ::
    IvyEvictInner(s,s',r)
  }

predicate IvyEvictInner(s: Variables, s': Variables, r:Reference)
  {
  && s.Ready? && s'.Ready?
  && (exists N: Node :: r in s.cache && s.cache[r] == N)
  //&& (forall L :: (r in s.ephemeralIndirectionTable.graph
  //          && r in s.ephemeralIndirectionTable.locs
  //          && s.ephemeralIndirectionTable.locs[r] == L) ==>
  //          (forall I :: I in s.outstandingBlockWrites ==> s.outstandingBlockWrites[I] != OutstandingWrite(r,L)))
  && !(r in s.ephemeralIndirectionTable.graph && !(r in s.ephemeralIndirectionTable.locs))
  && (forall R, N :: (R in s'.cache && s'.cache[R] == N) ==
               (R in s.cache && s.cache[R] == N && r != R))
  }



  lemma EvictImpliesIvyEvict(s: Variables, s': Variables, dop: DiskOp, vop: VOp, ref: Reference)
    requires Evict(s,s',dop,vop,ref)
    ensures IvyEvict(s,s')
  {

    // var r := ref;
    // var l := s.ephemeralIndirectionTable.locs[ref];
    //    forall I | I in s.outstandingBlockWrites ensures
    //      s.outstandingBlockWrites[I] != OutstandingWrite(r,l)
    //      {}

    //forall L ensures
    //(r in s.ephemeralIndirectionTable.graph &&
    // r in s.ephemeralIndirectionTable.locs &&
    // s.ephemeralIndirectionTable.locs[r] == L) ==>
    // (forall I :: I in s.outstandingBlockWrites ==> s.outstandingBlockWrites[I] != OutstandingWrite(r,L))
    //{
    //  if (r in s.ephemeralIndirectionTable.graph &&
    //      r in s.ephemeralIndirectionTable.locs &&
    //      s.ephemeralIndirectionTable.locs[r] == L)
    //  {
    //      forall I | I in s.outstandingBlockWrites
    //          ensures s.outstandingBlockWrites[I] != OutstandingWrite(r,L)
    //       {
    //          assert r in s.ephemeralIndirectionTable.graph;
    //          assert OutstandingWrite(r, s.ephemeralIndirectionTable.locs[r]) !in s.outstandingBlockWrites.Values;
    //          assert s.outstandingBlockWrites[I] != OutstandingWrite(r,L);
    //       }
    //  }
    //  else
    //  {}
    //}
    assert IvyEvictInner(s,s',ref);
  }

  // s.frozenIndirectionTable should be None? -- just through away old version
...
assert (forall ref :: forall loc : Location ::
  ...
  predicate Dirty(s: Variables, s': Variables, ref: Reference, block: Node)
  {
    ...
    // question: looks like req in cache is necessary to prevent page_in interferences
    && ref in s.ephemeralIndirectionTable.graph
    && s'.Ready?
