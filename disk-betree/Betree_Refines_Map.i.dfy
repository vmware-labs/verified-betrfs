...
module Betree_Refines_Map {
  ...
  lemma SuccQueryStepRefinesMap(k: DB.Constants, s: DB.Variables, s': DB.Variables, uiop: UIOp, key: Key, succKey: Key, succValue: Value, lookup1: Lookup, lookup2: Lookup)
    requires Inv(k, s)
    requires BetreeStepUI(BetreeSuccQuery(SuccQuery(key, succKey, succValue, lookup1, lookup2)), uiop)
    requires DBI.SuccQuery(k.bck, s.bcv, s'.bcv, key, succKey, succValue, lookup1, lookup2)
    requires Inv(k, s')
    ensures MS.NextStep(Ik(k), I(k, s), I(k, s'), uiop, MS.SuccStep(key, succKey, succValue))
  {
    if LookupKeyValue(lookup1, succKey, succValue) {
      LookupImpliesMap(k, s, succKey, succValue, lookup1);
    } else {
      LookupImpliesMap(k, s, succKey, succValue, lookup2);
    }

    forall mid | SeqComparison.lt(key, mid) && SeqComparison.lt(mid, succKey) && mid in I(k,s).view
    ...
    assert MS.Succ(Ik(k), I(k,s), I(k,s'), uiop, key, succKey, succValue);
  }
    ...
    ensures MS.Next(Ik(k), I(k, s), I(k, s'), uiop)
  {
    ...
    match betreeStep {
      ...
      case BetreeSuccQuery(q) => SuccQueryStepRefinesMap(k, s, s', uiop, q.key, q.succKey, q.succValue, q.lookup1, q.lookup2);
