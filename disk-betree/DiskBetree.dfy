...
abstract module DiskBetree {
  ...
  predicate IsSatisfyingLookup<Value>(k: Constants, s: Variables, key: Key, value: Value, lookup: Lookup) {
    ...
    && (forall i :: 0 <= i < |lookup| ==> lookup[i].accumulatedBuffer == (if i > 0 then lookup[i-1].accumulatedBuffer else []) + lookup[i].node.buffer[key])
  ...
  predicate Flush<Value>(k: Constants, s: Variables, s': Variables, parentref: BC.Reference, parent: Node, childref: BC.Reference, child: Node, newchildref: BC.Reference, sint: BC.Variables) {
  ...
  predicate Grow(k: Constants, s: Variables, s': Variables, oldroot: Node, newchildref: BC.Reference, sint: BC.Variables) {
    && BC.Read(k.bcc, s.bcv, BC.Root(k.bcc), oldroot)
    && var newchild := oldroot;
    && var newroot := Node(imap k :: newchildref, imap[]);
    && var allocop := BC.AllocOp(newchild, Successors(newchild), newchildref);
