...
module LinkedBetreeRefinement {
  ...
  function ILbl(lbl: TransitionLabel) : FilteredBetree.TransitionLabel
  {
    match lbl
      case QueryLabel(endLsn, key, value) => FilteredBetree.QueryLabel(endLsn, key, value)
      case PutLabel(puts) => FilteredBetree.PutLabel(puts)
      case QueryEndLsnLabel(endLsn) => FilteredBetree.QueryEndLsnLabel(endLsn)
      case FreezeAsLabel(stampedBetree) => FilteredBetree.FreezeAsLabel(
      ...
      case InternalLabel() => FilteredBetree.InternalLabel()
  }
  ...
  // wrapper
  function ILinkedBetree(linked: LinkedBetree) : (out: FilteredBetree.BetreeNode)
    requires linked.Acyclic()
    ensures out.WF()
  {
  ...
  function ILinkedBufferSeq(bufferSeq: BufferSeq, dv: BufferDiskView) : (out: BufferSeqMod.BufferSeq)
    requires bufferSeq.Valid(dv)
  {
    BufferSeqMod.BufferSeq(seq(bufferSeq.Length(), i requires 0 <= i < bufferSeq.Length() => dv.Get(bufferSeq.buffers[i])))
  }

  function IChildren(linked: LinkedBetree, ranking: Ranking) : seq<FilteredBetree.BetreeNode>
    requires linked.WF()
    requires linked.HasRoot()
    requires linked.ValidRanking(ranking)
  ...
  function ILinkedBetreeNode(linked: LinkedBetree, ranking: Ranking) : (out: FilteredBetree.BetreeNode)
    requires linked.WF()
    requires linked.ValidRanking(ranking)
    ensures out.WF()
  ...
  {
    if linked.root.None?
    then FilteredBetree.Nil
    else
      var node := linked.Root();
      var out := FilteredBetree.BetreeNode(ILinkedBufferSeq(node.buffers, linked.bufferDiskView), 
        node.pivotTable, IChildren(linked, ranking), node.flushedOffsets);
      assert out.WF() by {
        forall i:nat |
  ...
  // function IPath(path: Path) : (out: FilteredBetree.Path)
  //   requires path.linked.Acyclic()
  //   requires path.Valid()
  // {
  //   FilteredBetree.Path(ILinkedBetree(path.linked), path.key, path.depth)
  // }
  ...
  // function IReceiptLine(line: QueryReceiptLine) : FilteredBetree.QueryReceiptLine
  //   requires line.linked.Acyclic()
  // {
  //   FilteredBetree.QueryReceiptLine(
  //     ILinkedBetree(line.linked),
  //     line.result
  //   )
  // }

  // function IReceipt(receipt: QueryReceipt) : (out: FilteredBetree.QueryReceipt)
  //   requires receipt.Valid()
  // {
  //   FilteredBetree.QueryReceipt(
  //     receipt.key,
  ...
  // function IStepDefn(step: Step) : (out: FilteredBetree.Step)
  //   requires step.WF()
  //   requires StepPathRootAcyclic(step)
  // {
  //    match step {
  //     case QueryStep(receipt) => FilteredBetree.QueryStep(IReceipt(receipt))
  //     case PutStep() => FilteredBetree.PutStep()
  //     case QueryEndLsnStep() => FilteredBetree.QueryEndLsnStep()
  //     case FreezeAsStep() => FilteredBetree.FreezeAsStep()
  //     case InternalGrowStep(_) => FilteredBetree.InternalGrowStep()
  //     case InternalSplitStep(path, request, newAddrs, pathAddrs) =>
  //       FilteredBetree.InternalSplitStep(IPath(path), request)
  //     case InternalFlushStep(path, childIdx, _, _, _) =>
  //       var out := FilteredBetree.InternalFlushStep(IPath(path), childIdx);
  //       IPathValid(path);
  //       TargetCommutesWithI(path);
  //       out
  //     case InternalFlushMemtableStep(_) => FilteredBetree.InternalFlushMemtableStep()
  //     case InternalCompactStep(path, compactedBuffers, _, _) =>
  //       var out := FilteredBetree.InternalCompactStep(IPath(path), compactedBuffers);
  //       IPathValid(path);
  //       TargetCommutesWithI(path);
  //       out
  //     case InternalNoOpStep() => FilteredBetree.InternalNoOpStep()
  //   }
  // }
  ...
  //           assert IChildren(child, target.TheRanking())[0] == FilteredBetree.Nil;  // trigger
  //         } else {
  ...
  // function IStep(step: Step) : (out: FilteredBetree.Step)
  //   requires step.WF()
  //   requires StepPathRootAcyclic(step)
  //   ensures out.WF()
  // {
  ...
  //     assert FilteredBetree.BetreeNode(linked.Root().buffers, linked.Root().pivotTable, IChildren(linked, linked.TheRanking())).KeyInDomain(key);
  //     calc {
  //       FilteredBetree.Nil;
  //       FilteredBetree.BetreeNode(linked.Root().buffers, linked.Root().pivotTable, IChildren(linked, linked.TheRanking())).Child(key);
  //     }
  //   }
  // }
  ...
  function IStampedBetree(stampedBetree: StampedBetree) : (out: FilteredBetree.StampedBetree)
  ...
  function I(v: Variables) : (out: FilteredBetree.Variables)
    requires v.WF()
    requires v.linked.Acyclic()
  {
    FilteredBetree.Variables(v.memtable, ILinkedBetree(v.linked))
  }
  ...
  //   ensures FilteredBetree.NextStep(I(v), I(v'), ILbl(lbl), IStep(step))
  // {
  //   InvNext(v, v', lbl);
  ...
  //     == FilteredBetree.CompactedNode(ILinkedBetree(target), compactedBuffers);
  // {
  ...
  //     == |FilteredBetree.CompactedNode(ILinkedBetree(target), compactedBuffers).children|;

  //   forall i | 0 <= i < |ILinkedBetree(replacement).children|
  //   ensures ILinkedBetree(replacement).children[i] == FilteredBetree.CompactedNode(ILinkedBetree(target), compactedBuffers).children[i]
  //   {
  ...
  //       FilteredBetree.CompactedNode(ILinkedBetree(target), compactedBuffers).children[i];
  //     }
  //   }
  // }
  ...
  //     istep.path.Substitute(FilteredBetree.CompactedNode(ILinkedBetree(step.path.Target()), step.compactedBuffers));
  //       { TargetCommutesWithI(step.path); }
  //     istep.path.Substitute(FilteredBetree.CompactedNode(istep.path.Target(), istep.compactedBuffers));
  //   }
  // }
  ...
  //         FilteredBetree.BetreeNode(subroot.buffers.PushBufferSeq(movedBuffers), subroot.pivotTable, IChildren(replacement.ChildAtIdx(i), replacementRanking));
  //         {
  ...
  //         }
  //         FilteredBetree.BetreeNode(subroot.buffers.PushBufferSeq(movedBuffers), subroot.pivotTable, IChildren(target.ChildAtIdx(i), replacementRanking));
  ...
  //     ensures ILinkedBetreeNode(replacement.ChildAtIdx(i), newRanking) == FilteredBetree.Nil 
  //     {}
  //   }
  // }
  ...
  //   assert FilteredBetree.WFChildren(itarget.children);  // trigger
  ...
  //         }
  //       } else {
  //         assert IChildren(LinkedBetree(Some(step.newAddrs.right), target'.diskView), ranking')[0] == FilteredBetree.Nil;  // trigger
  //       }
  //       ILinkedBetreeIgnoresRanking(target'.ChildAtIdx(idx), target'.TheRanking(), ranking');
  //       ChildAtIdxCommutesWithI(target', idx, target'.TheRanking());
  //     } else {
  ...
  //   TargetCommutesWithI(step.path);
  //   BuildTightPreservesInterpretation(step.path.Substitute(replacement, step.pathAddrs));
  //   // assert FilteredBetree.InternalSplit(I(v), I(v'), ILbl(lbl), IStep(step));
  // }
  ...
  lemma NextRefines(v: Variables, v': Variables, lbl: TransitionLabel)
    ...
    // ensures v'.WF()
    ensures Inv(v')
    ensures FilteredBetree.Next(I(v), I(v'), ILbl(lbl))
  {
    InvNext(v, v', lbl);
    assume false;
    // var step: Step :| NextStep(v, v', lbl, step);
    // match step {
    //   case QueryStep(receipt) => {
    //     assert FilteredBetree.NextStep(I(v), I(v'), ILbl(lbl), IStep(step));
    //   }
    //   case PutStep() => {
    //     assert FilteredBetree.NextStep(I(v), I(v'), ILbl(lbl), IStep(step));
    //   }
