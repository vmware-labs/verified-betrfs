            ...
            split_branch.has_root(),
    {
        ...
        match self {
            Self::SplitLeaf { pivot } => {
                ...
                &&& 0 < Key::largest_lt(root.get_Leaf_keys(), pivot) + 1
...
impl DiskView {
    ...
    pub open spec(checked) fn node_has_valid_child_address(self, node: Node) -> bool {
        node is Index ==> forall|idx|
            0 <= idx < node.get_Index_children().len() ==> self.valid_address(
                #[trigger]
                node.get_Index_children()[idx],
            )
    }
    ...
    pub open spec(checked) fn no_dangling_address(self) -> bool {
        forall|addr|
            #[trigger]
            self.entries.contains_key(addr) ==> self.node_has_valid_child_address(
                self.entries[addr],
            )
    }
            ...
            ranking.contains_key(addr),
    {
        ...
        forall|child_idx: nat|
            #[trigger]
            node.valid_child_index(child_idx) ==> {
                &&& ranking.contains_key(
                    node.get_Index_children()[child_idx as int],
                )  // ranking is closed
            ...
            self.wf(),
    {
        forall|addr|
            #[trigger]
            ranking.contains_key(addr) && self.entries.contains_key(addr)
...
impl LinkedBranch {
        ...
        decreases self.get_rank(ranking), 1int,
    {
        self.root() is Index ==> {
            &&& forall|i|
                ...
                &&& forall|i|
                    0 <= i < self.root().get_Index_children().len() - 1
    ...
    fn child_all_keys_in_range_internal(self, ranking: Ranking, i: nat) -> bool
        recommends
            self.wf(),
            self.valid_ranking(ranking),
            self.root() is Index,
    ...
    pub open spec(checked) fn children_keys(self, ranking: Ranking, i: nat) -> Set<Key>
        recommends
            ...
            self.root() is Index,
            0 <= i <= self.root().get_Index_children().len(),
        decreases self.get_rank(ranking), 0int, self.root().get_Index_children().len() - i,
        when {
        &&& self.root() is Index
        &&& 0 <= i <= self.root().get_Index_children().len()
        &&& self.root().valid_child_index(i) ==> self.child_at_idx(i).get_rank(ranking)
    ...
    pub open spec   /*XXX (checked)*/
    fn all_keys_below_bound(self, i: int, ranking: Ranking) -> bool
        recommends
            self.wf(),
            self.valid_ranking(ranking),
            self.root() is Index,
        ...
        decreases self.get_rank(ranking),
    {
        // Need valid ranking implies child has valid ranking to restore checked
        forall|key|
            self.child_at_idx(i as nat).all_keys(ranking).contains(key) ==> Key::lt(
                key,
                self.root().get_Index_pivots()[i],
            )
    }

    pub open spec   /*XXX (checked)*/
    fn all_keys_above_bound(self, i: int, ranking: Ranking) -> bool
        recommends
            self.wf(),
            self.valid_ranking(ranking),
            self.root() is Index,
        ...
        decreases self.get_rank(ranking),
    {
        // Need valid ranking implies child has valid ranking to restore checked
        forall|key|
            self.child_at_idx(i as nat).all_keys(ranking).contains(key) ==> #[trigger]
            Key::lte(self.root().get_Index_pivots()[i - 1], key)
    }
        ...
        decreases self.get_rank(ranking), 1int,
    {
        if !self.has_root()  {
            ...
            let subtree_addrs = Seq::new(
                num_children,
                |i: int| self.child_reachable_addrs_using_ranking(ranking, i as nat),
            );
            union_seq_of_sets(subtree_addrs).insert(self.root)
        }
    }

    pub open spec(checked) fn child_reachable_addrs_using_ranking(
        self,
        ranking: Ranking,
        i: nat,
    ) -> Set<Address>
    ...
    pub open spec   /*XXX (checked)*/
    fn query_internal(self, key: Key, ranking: Ranking) -> Message
        recommends
            self.wf(),
            self.valid_ranking(ranking),
        decreases self.get_rank(ranking),
    ...
    pub open spec   /*XXX (checked)*/
    fn insert_leaf(self, key: Key, msg: Message) -> LinkedBranch
        recommends
            self.wf(),
            self.root() is Leaf,
    {
        ...
        let new_node = if 0 <= llte && node.get_Leaf_keys()[llte] == key {
            Node::Leaf { keys: node.get_Leaf_keys(), msgs: node.get_Leaf_msgs().update(llte, msg) }
        } else {
            Node::Leaf {
                keys: node.get_Leaf_keys().insert(llte + 1, key),
                msgs: node.get_Leaf_msgs().insert(llte + 1, msg),
            }
        };
    ...
    pub open spec   /*XXX (checked)*/
    fn insert(self, key: Key, msg: Message, path: Path) -> LinkedBranch
        recommends
            self.wf(),
            path.valid(),
            path.branch == self,
    ...
    pub open spec(checked) fn append_to_new_leaf(
        self,
        new_keys: Seq<Key>,
        new_msgs: Seq<Message>,
    ) -> LinkedBranch
        recommends
            self.wf(),
            self.root() is Leaf,
    ...
    pub open spec   /*XXX (checked)*/
    fn append(self, keys: Seq<Key>, msgs: Seq<Message>, path: Path) -> LinkedBranch
        recommends
            self.wf(),
            path.valid(),
            path.branch == self,
    ...
    pub open spec(checked) fn split_leaf(self, split_arg: SplitArg, right_root_addr: Address) -> (
        LinkedBranch,
        LinkedBranch,
    )
        recommends
            ...
            self.disk_view.is_fresh(set!{right_root_addr}),
    {
        ...
        let new_disk_view = self.disk_view.modify_disk(self.root, left_root).modify_disk(
            right_root_addr,
            right_root,
        );
    ...
    pub open spec   /*XXX (checked)*/
    fn split_index(self, split_arg: SplitArg, right_root_addr: Address) -> (
        LinkedBranch,
        LinkedBranch,
    )
        recommends
            ...
            self.disk_view.is_fresh(set!{right_root_addr}),
    {
        ...
        let right_root = self.sub_index(
            pivot_index + 1,
            self.root().get_Index_children().len() as int,
        );
        let new_disk_view = self.disk_view.modify_disk(self.root, left_root).modify_disk(
            right_root_addr,
            right_root,
    ...
    pub open spec(checked) fn split_node(self, split_arg: SplitArg, right_root_addr: Address) -> (
        LinkedBranch,
        LinkedBranch,
    )
        recommends
    ...
    pub open spec   /*XXX (checked)*/
    fn can_split_child_of_index(self, split_arg: SplitArg, new_child_addr: Address) -> bool {
    ...
    pub open spec   /*XXX (checked)*/
    fn split_child_of_index(self, split_arg: SplitArg, new_child_addr: Address) -> LinkedBranch
        recommends
            self.can_split_child_of_index(split_arg, new_child_addr),
    {
        // Need route ensures to restore checked
        let pivot = split_arg.get_pivot();
        let child_idx = self.root().route(pivot) + 1;
        let (left_branch, right_branch) = self.child_at_idx(child_idx as nat).split_node(
            split_arg,
            new_child_addr,
        );
        ...
        let new_disk_view = self.disk_view.merge_disk(left_branch.disk_view).modify_disk(
            self.root,
            new_root,
        );
        LinkedBranch { root: self.root, disk_view: new_disk_view }
    ...
    pub open spec(checked) fn split(
        self,
        addr: Address,
        path: Path,
        split_arg: SplitArg,
...
pub open spec(checked) fn empty_linked_branch(root: Address) -> LinkedBranch {
    LinkedBranch {
        root: root,
        disk_view: empty_disk().modify_disk(root, Node::Leaf { keys: seq![], msgs: seq![] }),
    }
}
...
impl Path {
            ...
            self.branch.root() is Index,
    {
        ...
        Path {
            branch: self.branch.child_at_idx(r as nat),
            key: self.key,
