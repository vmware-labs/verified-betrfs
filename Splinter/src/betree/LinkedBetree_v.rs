...
impl BetreeNode {
    ...
    pub open spec(checked) fn my_domain(self) -> Domain
        recommends
            self.wf(),
    {
        Domain::Domain { start: self.pivots[0], end: self.pivots.pivots.last() }
    }
    ...
    fn flushed_ofs(self, key: Key) -> nat
        recommends
    ...
    pub open spec(checked) fn compact_key_range(
        self,
        start: nat,
        end: nat,
        k: Key,
        buffer_dv: BufferDiskView,
    ) -> bool
        recommends
            self.wf(),
            start < end <= self.buffers.len(),
    {
        ...
        &&& exists|buffer_idx|
            self.buffers.slice(start as int, end as int).key_in_buffer_filtered(
                buffer_dv,
    ...
    pub open spec(checked) fn compact_key_value(
        self,
        start: nat,
        end: nat,
        k: Key,
    ...
    {
        let from = if self.flushed_ofs(k) <= start {
            0
        } else {
            self.flushed_ofs(k) - start
        };
    ...
    pub open spec   /*XXX(checked)*/
    fn child_ptr(self, key: Key) -> Pointer
        recommends
            self.key_in_domain(key),
    {
...
impl DiskView {
            ...
            node.wf(),
    {
        forall|i|
            #[trigger]
            node.valid_child_index(i) ==> self.is_nondangling_ptr(node.children[i as int])
    }
            ...
            self.node_has_nondangling_child_ptrs(node),
    {
        ...
        &&& child_ptr.is_Some() ==> self.entries[child_ptr.unwrap()].my_domain()
    ...
    pub open spec(checked) fn node_children_respects_rank(
        self,
        ranking: Ranking,
        addr: Address,
    ) -> bool
            ...
            ranking.contains_key(addr),
    {
        ...
        &&& forall|idx|
            #[trigger]
            node.valid_child_index(idx) && node.children[idx as int].is_Some() ==> ({
    ...
    pub open spec(checked) fn valid_ranking(self, ranking: Ranking) -> bool {
        ...
        &&& forall|addr|
            ...
            ranking.contains_key(addr) ==> self.node_children_respects_rank(ranking, addr)
    }
    ...
    pub open spec(checked) fn no_dangling_buffer_ptr(self, buffer_dv: BufferDiskView) -> bool {
        forall|addr|
            self.entries.contains_key(addr) ==> #[trigger]
            self.entries[addr].buffers.valid(buffer_dv)
    }
}
...
impl LinkedBetree {
    ...
    {
        LinkedBetree {
            root: self.root().children[idx as int],
            dv: self.dv,
    ...
    pub open spec(checked) fn can_recurse_for_reachable(
        self,
        ranking: Ranking,
        child_idx: nat,
    ) -> bool {
    ...
    pub open spec(checked) fn reachable_addrs_using_ranking_recur(
        self,
        ranking: Ranking,
        child_idx: nat,
    ) -> Set<Address>
        ...
        when self.can_recurse_for_reachable(ranking, child_idx)
    ...
    pub open spec   /*XXX (checked)*/
    fn reachable_buffer(self, addr: Address, buffer_addr: Address) -> bool
        recommends
            self.acyclic(),
    {
    ...
    pub open spec(checked) fn push_memtable(
        self,
        memtable: Memtable,
        new_addrs: TwoAddrs,
    ) -> LinkedBetree
    ...
    pub open spec(checked) fn split_parent(
        self,
            ...
            self.can_split_parent(request),
    {
        match request {
            SplitRequest::SplitLeaf { child_idx, split_key } => {
                let (new_left_child, new_right_child) = self.child_at_idx(
                    child_idx,
                ).root().split_leaf(split_key);
                let new_children = self.root().children.update(
                    child_idx as int,
                    Some(new_addrs.left),
                ...
                let new_dv = self.dv.modify_disk(new_addrs.left, new_left_child).modify_disk(
                    new_addrs.right,
                    new_right_child,
            ...
            SplitRequest::SplitIndex { child_idx, child_pivot_idx } => {
                let (new_left_child, new_right_child) = self.child_at_idx(
                    child_idx,
                ).root().split_index(child_pivot_idx);
                let new_children = self.root().children.update(
                    child_idx as int,
                    Some(new_addrs.left),
                ).insert(child_idx as int + 1, Some(new_addrs.right));
                ...
                let new_parent = BetreeNode {
                    buffers: self.root().buffers,
                    pivots: self.root().pivots.insert(
                        child_idx as int + 1,
    ...
    pub open spec(checked) fn can_flush(self, child_idx: nat, buffer_gc: nat) -> bool {
        ...
        &&& self.root().flushed.update(child_idx as int, self.root().buffers.len()).all_gte(
            buffer_gc,
        )
    }

    pub open spec   /*XXX(checked)*/
    fn flush(self, child_idx: nat, buffer_gc: nat, new_addrs: TwoAddrs) -> LinkedBetree
        recommends
            ...
            self.is_fresh(new_addrs.repr()),
    {
        ...
        let new_dv = self.dv.modify_disk(new_addrs.addr1, new_root).modify_disk(
            new_addrs.addr2,
            new_child,
        );
    ...
    pub open spec(checked) fn can_compact(
        self,
        start: nat,
        end: nat,
        ...
        &&& forall|k|
            #![auto]
            compacted_buffer.map.contains_key(k) <==> self.root().compact_key_range(
                start,
                end,
                k,
                self.buffer_dv,
            ...
            &&& forall|k|
                #![auto]
                compacted_buffer.map.contains_key(k) ==> compacted_buffer.query(k)
                    == self.root().compact_key_value(start, end, k, self.buffer_dv)
    }
...
impl QueryReceipt {
    pub open spec(checked) fn structure(self) -> bool {
        ...
        &&& forall|i: nat|
            #![auto]
            i < self.lines.len() ==> self.lines[i as int].linked.dv == self.linked.dv
            &&& forall|i: nat|
                #![auto]
                i < self.lines.len() ==> self.lines[i as int].linked.buffer_dv
                    == self.linked.buffer_dv
                &&& forall|i: nat|
                    #![auto]
                    i < self.lines.len() ==> self.lines[i as int].linked.has_root() <==> i
                        < self.lines.len() - 1
            ...
            self.structure(),
    {
        &&& forall|i: nat|
            #![auto]
            i < self.lines.len() ==> self.lines[i as int].wf()
            &&& forall|i: nat|
                #![auto]
                i < self.lines.len() ==> self.lines[i as int].linked.acyclic()
                &&& forall|i: nat|
                    #![auto]
                    i < self.lines.len() - 1 ==> self.lines[i as int].linked.root().buffers.valid(
                        self.linked.buffer_dv,
                    )
                    &&& forall|i: nat|
                        #![auto]
                        i < self.lines.len() - 1 ==> self.node(i as int).key_in_domain(self.key)
    }
    ...
    pub open spec   /*XXX(checked)*/
    fn result_linked_at(self, i: int) -> bool
        recommends
            self.structure(),
            self.all_lines_wf(),
            0 <= i < self.lines.len() - 1,
...
impl Path {
    ...
    pub open spec   /*XXX(checked)*/
    fn can_substitute(self, replacement: LinkedBetree, path_addrs: PathAddrs) -> bool {
        &&& self.valid()
    ...
    pub open spec   /*XXX (checked)*/
    fn substitute(self, replacement: LinkedBetree, path_addrs: PathAddrs) -> LinkedBetree
        recommends
        ...
        decreases self.depth, 1nat,
    {
        if self.depth == 0 {
            ...
            let subtree = self.subpath().substitute(
                replacement,
