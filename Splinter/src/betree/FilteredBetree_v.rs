...
impl BetreeNode {
            ...
            self.is_Node(),
    {
        &&& forall|i|
            ((#[trigger]
            self.valid_child_index(i)) && self.get_Node_children()[i as int].is_Node()
        ...
        when self.is_Node()
    {
        &&& (forall|i|
            (#[trigger]
            self.valid_child_index(i)) ==> self.get_Node_children()[i as int].wf())
    }
    ...
    pub open spec(checked) fn is_index(self) -> bool {
        &&& self.is_Node()
        &&& forall|i|
            0 <= i < self.get_Node_children().len() ==> (#[trigger]
            self.get_Node_children()[i]).is_Node()
    }
        ...
        match request {
            SplitRequest::SplitLeaf { child_idx, split_key } => {
                ...
                BetreeNode::Node {
                    ...
                    children: self.get_Node_children().update(idx, new_left_child).insert(
                        idx + 1,
                        new_right_child,
                    ),
    ...
    pub open spec(checked) fn can_flush(self, child_idx: nat, buffer_gc: nat) -> bool {
        ...
        &&& self.is_Node()
        &&& self.valid_child_index(child_idx)
        &&& self.get_Node_flushed().update(child_idx as int, self.get_Node_buffers().len()).all_gte(
            buffer_gc,
        )
    }

    pub open spec   /*XXX (checked)*/
    fn flush(self, child_idx: nat, buffer_gc: nat) -> BetreeNode
        recommends
            self.can_flush(child_idx, buffer_gc),
    {
        ...
        let new_child = self.get_Node_children()[idx].promote(
            self.child_domain(child_idx),
        ).extend_buffer_seq(buffers_to_child);
            ...
            start < end <= self.get_Node_buffers().len(),
    {
        ...
        &&& exists|buffer_idx|
            self.get_Node_buffers().slice(start as int, end as int).key_in_buffer_filtered(
    ...
    pub open spec fn can_compact(self, start: nat, end: nat, compacted_buffer: Buffer) -> bool {
        ...
        &&& forall|k|
            #![auto]
            compacted_buffer.map.contains_key(k) <==> self.compact_key_range(start, end, k)
            &&& forall|k|
                #![auto]
                compacted_buffer.map.contains_key(k) ==> ({
                    let from = if self.flushed_ofs(k) <= start {
                        0
                    } else {
                        self.flushed_ofs(k) - start
                    };
                    &&& compacted_buffer.query(k) == self.get_Node_buffers().slice(
                        start as int,
                        end as int,
                    ).query_from(k, from)
                })
    ...
    pub open spec   /*XXX(checked)*/
    fn compact(self, start: nat, end: nat, compacted_buffer: Buffer) -> BetreeNode
        recommends
            self.can_compact(start, end, compacted_buffer),
    {
        ...
        BetreeNode::Node {
            buffers: self.get_Node_buffers().update_subrange(
                start as int,
                end as int,
                compacted_buffer,
            ),
    ...
    pub open spec   /*XXX(checked)*/
    fn child(self, key: Key) -> BetreeNode
        recommends
            self.wf(),
            self.is_Node(),
            self.key_in_domain(key),
...
impl QueryReceipt {
    ...
    pub open spec(checked) fn all_lines_wf(self) -> bool {
        ...
        &&& (forall|i: nat|
            #![auto]
            i < self.lines.len() - 1 ==> self.lines[i as int].node.key_in_domain(self.key))
    }
...
impl Path {
    ...
    pub open spec   /*XXX (checked)*/
    fn replaced_children(self, replacement: BetreeNode) -> Seq<BetreeNode>
        recommends
