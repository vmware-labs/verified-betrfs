...
impl BufferSeq {
    ...
    pub open spec(checked) fn update_subrange(
        self,
        start: int,
        end: int,
        new_buffer: Buffer,
            ...
            0 <= start < end <= self.len(),
    {
        ...
        BufferSeq {
            buffers: self.buffers.subrange(0, start) + s + self.buffers.subrange(
                end,
                self.len() as int,
            ),
        }
    ...
    fn i_filtered_from(self, offset_map: OffsetMap, idx: int) -> Buffer
        recommends
            offset_map.is_total(),
            0 <= idx <= self.len(),
        decreases self.len() - idx,
    ...
    pub open spec(checked) fn key_in_buffer_filtered(
        self,
    ...
    pub proof fn query_agrees_with_i(self, k: Key, start: int)
        requires
            0 <= start <= self.len(),
        ensures
            self.i_from(start).map.contains_key(k) ==> self.query_from(k, start) == self.i_from(
                start,
            ).map[k],
            !self.i_from(start).map.contains_key(k) ==> self.query_from(k, start) == (
            Message::Update { delta: nop_delta() }),
        decreases self.len() - start,
    {
        if start < self.len() {
    ...
    pub proof fn i_from_domain(self, idx: int)
        ...
        ensures
            forall|k|
                self.i_from(idx).map.contains_key(k) <==> exists|buffer_idx|
        ...
        decreases self.len() - idx,
    {
        assert forall|k|
            #[trigger]
            self.i_from(idx).map.contains_key(k) <==> exists|buffer_idx|
                self.key_in_buffer(idx, k, buffer_idx) by {
    ...
    pub proof fn i_filtered_from_domain(self, offset_map: OffsetMap, idx: int)
        ...
        ensures
            forall|k|
                self.i_filtered_from(offset_map, idx).map.contains_key(k) <==> exists|buffer_idx|
        ...
        decreases self.len() - idx,
    {
        ...
        assert forall|k|
            #[trigger]
            result.map.contains_key(k) <==> exists|buffer_idx|
                self.key_in_buffer_filtered(offset_map, idx, k, buffer_idx) by {
            if result.map.contains_key(k) {
                ...
                if self.key_in_buffer_filtered(offset_map, idx, k, idx) {
                    ...
                    let buffer_idx = choose|buffer_idx|
                        self.key_in_buffer_filtered(offset_map, next_idx, k, buffer_idx);
    ...
    pub proof fn query_from_same_as_i_filtered(self, k: Key, buffer_idx: int, offset_map: OffsetMap)
        ...
        ensures
            ({
                ...
                &&& start <= buffer_idx ==> self.i_filtered_from(offset_map, buffer_idx).query(k)
                    == self.query_from(k, buffer_idx)
                &&& start > buffer_idx ==> self.i_filtered_from(offset_map, buffer_idx).query(k)
                    == self.query_from(k, start)
            }),
    ...
    pub proof fn common_buffer_seqs(
        a: BufferSeq,
        b: BufferSeq,
        a_start: int,
        b_delta: int,
    ...
    pub proof fn extend_buffer_seq_lemma(top: BufferSeq, bottom: BufferSeq, key: Key, start: int)
        requires
            0 <= start <= bottom.len(),
        ensures
            bottom.extend(top).query_from(key, start) == bottom.query_from(key, start).merge(
                top.query(key),
            ),
    ...
    pub proof fn not_present_query_lemma(self, k: Key, start: int)
        ...
        ensures
            self.query_from(k, start) == (Message::Update { delta: nop_delta() }),
        decreases self.len() - start,
    {
        if start < self.len() {
