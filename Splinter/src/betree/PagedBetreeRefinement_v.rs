...
impl BetreeNode {
    ...
    fn build_query_receipt(self, key: Key) -> QueryReceipt
        recommends
            self.wf(),
        decreases self,
        when self.wf()
        ...
        decreases self,
    {
        if self.is_Node() {
            ...
            let receipt = QueryReceipt {
                key: key,
                root: self,
                lines: seq![line] + child_receipt.lines,
            };
            assert forall|i: int| 0 < i < receipt.lines.len() - 1 implies ({
    ...
    pub proof fn memtable_distributes_over_betree(self, memtable: Memtable)
        requires
            self.wf(),
        ensures
            map_apply(memtable, self.i()) == self.push_memtable(memtable).value.i(),
    {
    ...
    pub proof fn push_empty_memtable_refines(self, memtable: Memtable)
        requires
            ...
            memtable.is_empty(),
        ensures
            i_stamped_betree(Stamped { value: self, seq_end: memtable.seq_end })
    ...
    pub proof fn merge_buffer_lemma(self, new_buffer: Buffer, key: Key)
        requires
            self.wf(),
        ensures
            self.promote().merge_buffer(new_buffer).i_at(key) == self.i_at(key).merge(
                new_buffer.query(key),
            ),
    {
    }
    ...
    pub proof fn filter_buffer_and_children_wf(self, filter: Set<Key>)
        requires
            self.wf(),
        ensures
            self.filter_buffer_and_children(filter).wf(),
    {
        if self.is_Node() {
    ...
    pub proof fn split_wf(self, left_keys: Set<Key>, right_keys: Set<Key>)
        requires
            ...
            self.is_Node(),
        ensures
            self.split(left_keys, right_keys).wf(),
    {
    ...
    pub proof fn apply_filter_equivalence(self, filter: Set<Key>, key: Key)
        requires
            ...
            filter.contains(key),
        ensures
            self.filter_buffer_and_children(filter).i_at(key) == self.i_at(key),
    {
...
impl QueryReceipt {
    ...
    pub proof fn drop_first_valid(self)
        requires
            ...
            1 < self.lines.len(),
        ensures
            self.drop_first().valid(),
    {
        let out = self.drop_first();
        assert(self.child_linked_at(0));
        assert forall|i: int| 0 <= i < out.lines.len() - 1 implies ({
...
impl Path {
    ...
    pub proof fn substitute_receipt_equivalence(self, replacement: BetreeNode, key: Key)
        ...
        ensures
            self.node.i_at(key) == self.substitute(replacement).i_at(key),
        decreases self.routing.len(),
    {
            ...
            self.node.i() == self.substitute(replacement).i(),
    {
        ...
        assert forall|k: Key|
            (#[trigger]
            self.node.i_at(k)) == self.substitute(replacement).i_at(k) by {
...
impl PagedBetree::Label {
    pub open spec(checked) fn i(self) -> AbstractMap::Label {
        match self {
            PagedBetree::Label::Query { end_lsn, key, value } => AbstractMap::Label::QueryLabel {
                end_lsn: end_lsn,
                key: key,
                value: value,
            },
            PagedBetree::Label::Put { puts } => AbstractMap::Label::PutLabel { puts: puts },
            PagedBetree::Label::FreezeAs { stamped_betree } => AbstractMap::Label::FreezeAsLabel {
...
pub proof fn composite_single_put(puts1: MsgHistory, puts2: MsgHistory, stamped_map: StampedMap)
    ...
    ensures
        puts1.concat(puts2).apply_to_stamped_map(stamped_map) == puts2.apply_to_stamped_map(
        ...
        ),
{
    ...
    assert(puts2.discard_recent(last_lsn).apply_to_stamped_map(
        puts1.apply_to_stamped_map(stamped_map),
    ) == puts1.apply_to_stamped_map(stamped_map));
...
impl PagedBetree::State {
    ...
    pub proof fn init_refines(self, stamped_betree: StampedBetree)
        requires
            PagedBetree::State::initialize(self, stamped_betree),
        ensures
            self.inv(),
            AbstractMap::State::initialize(self.i(), i_stamped_betree(stamped_betree)),
    {
    ...
    pub proof fn apply_single_put_is_map_plus_history(self, post: Self, puts: MsgHistory)
        requires
            ...
            post.root == self.root,
        ensures
            post.i().stamped_map == MsgHistory::map_plus_history(self.i().stamped_map, puts),
    {
        ...
        let sub_map_b = puts.discard_recent(puts.seq_start).apply_to_stamped_map(
            self.i().stamped_map,
        ).value;
        ...
        decreases puts.len(),
    {
        if 0 < puts.len() {
            ...
            let intermediate_post = PagedBetree::State {
                root: self.root,
    ...
    pub proof fn internal_flush_memtable_noop(self, post: Self, lbl: PagedBetree::Label)
        requires
            self.inv(),
            PagedBetree::State::internal_flush_memtable(self, post, lbl),
        ensures
            post.inv(),
            AbstractMap::State::next(self.i(), post.i(), lbl.i()),
    {
    ...
    pub proof fn internal_split_noop(
        self,
        post: Self,
        ...
        requires
            self.inv(),
            PagedBetree::State::internal_split(self, post, lbl, path, left_keys, right_keys),
        ensures
            post.inv(),
            AbstractMap::State::next(self.i(), post.i(), lbl.i()),
    {
        ...
        assert forall|k: Key| target.i_at(k) == top.i_at(k) by {
            if left_keys.contains(k) {
    ...
    pub proof fn internal_flush_noop(
        self,
        post: Self,
        lbl: PagedBetree::Label,
        path: Path,
        ...
        requires
            self.inv(),
            PagedBetree::State::internal_flush(self, post, lbl, path, down_keys),
        ensures
            post.inv(),
            AbstractMap::State::next(self.i(), post.i(), lbl.i()),
    {
            ...
            AbstractMap::State::next(self.i(), post.i(), lbl.i()),
    {
        ...
        match choose|step| PagedBetree::State::next_by(self, post, lbl, step) {
            ...
            PagedBetree::Step::internal_split(path, left_keys, right_keys) => {
                self.internal_split_noop(post, lbl, path, left_keys, right_keys);
            },
