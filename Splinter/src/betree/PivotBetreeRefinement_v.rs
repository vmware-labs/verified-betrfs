...
impl BetreeNode {
    ...
    pub proof fn i_children_seq_same(self, other: BetreeNode, start: int)
        requires
            ...
            other.wf(),
            other.is_Node(),
            other.get_Node_children() == self.get_Node_children(),
        ensures
            self.i_children_seq(start) == other.i_children_seq(start),
    ...
    pub proof fn i_children_lemma(self)
        ...
        ensures
            forall|k: Key|
                {
                    (#[trigger]
                    self.i_children().map[k]).wf() && self.i_children().map[k] == self.i_child(k)
                },
        decreases self, 1nat,
    {
        let seq_result = self.i_children_seq(0);
    ...
    pub proof fn empty_root_refines()
        ensures
            Self::empty_root(total_domain()).i() == PagedBetree_v::BetreeNode::empty_root(),
    {
            ...
            self.can_split_parent(request),
    {
        ...
        match request {
            ...
            SplitRequest::SplitIndex {
                child_idx,
                child_pivot_idx,
            } => old_child.get_Node_pivots().pivots[child_pivot_idx as int],
        }
    }
            ...
            self.can_split_parent(request),
    {
        ...
        let left_keys = Set::new(
            |k| child_domain.contains(k) && Element::lt(to_element(k), split_element),
        );
    ...
    pub proof fn split_keys_agrees_with_domains(self, request: SplitRequest)
        ...
        ensures
            ({
                ...
                let split_element = self.split_element(request);
                let left_domain = Domain::Domain {
                    start: child_domain.get_Domain_start(),
                    end: split_element,
                };
                let right_domain = Domain::Domain {
                    start: split_element,
            ...
            }),
    {
        ...
        assert forall|k: Key| #[trigger] left_keys.contains(k) <==> left_domain.contains(k) by {
        ...
        assert forall|k: Key| #[trigger] right_keys.contains(k) <==> right_domain.contains(k) by {
    ...
    pub proof fn split_commutes_with_i_left(self, request: SplitRequest, key: Key)
        ...
        ensures
            self.split_parent(request).i_children().map[key] == self.i().split(
            ...
            ).child(key),
    {
        ...
        PivotTable::route_lemma_auto();
        self.split_parent(request).get_Node_pivots().route_is_lemma(
            key,
        ...
        if request.is_SplitLeaf() {
            ...
            assert forall|k|
                #[trigger]
                a.get_Node_children().map[k] == b.i().get_Node_children().map[k] by {}
            ...
            ).child(key),
    {
        ...
        self.split_parent(request).get_Node_pivots().route_is_lemma(
            key,
            request.get_child_idx() as int + 1,
        );
            ...
            }),
    {
        self.split_parent_wf(request);
        let (left_keys, right_keys) = self.split_keys(request);
        assert forall|k: Key| self.my_domain().contains(k) implies (#[trigger]
        self.i().split(left_keys, right_keys).get_Node_children().map[k]) == self.split_parent(
            request,
        ).i_children().map[k] by {
        ...
        assert(self.i().split(left_keys, right_keys).get_Node_children().map =~= self.split_parent(
            request,
        ).i_children().map);
    }
    ...
    pub proof fn promote_and_merge_wf(self, domain: Domain, buffer: Buffer)
        requires
            ...
            domain.is_Domain(),
        ensures
            self.promote(domain).merge_buffer(buffer).wf(),
    {
        let result = self.promote(domain).merge_buffer(buffer);
        assert(self.promote(domain).wf());
        assert forall|i|
            #[trigger]
            result.valid_child_index(i) ==> self.promote(domain).valid_child_index(i) by {}
        assert(result.wf());
    }

    pub proof fn flush_wf(self, child_idx: nat)
        requires
            self.can_flush(child_idx),
        ensures
            self.flush(child_idx).wf(),
    {
    ...
    pub proof fn promote_and_merge_commutes_with_i(self, domain: Domain, new_buffer: Buffer)
        requires
            ...
            domain.is_Domain(),
        ensures
            self.promote(domain).merge_buffer(new_buffer).i() == self.i().promote().merge_buffer(
                new_buffer,
            ),
    {
        self.i_wf();
        ...
        if self.is_Node() {
            assert forall|i|
                #[trigger]
                a.valid_child_index(i) ==> self.promote(domain).valid_child_index(i) by {}
    ...
    pub proof fn flush_commutes_with_i(self, child_idx: nat)
        requires
            self.can_flush(child_idx),
        ensures
            self.i().flush(self.child_domain(child_idx).key_set()) == self.flush(child_idx).i(),
    {
        ...
        assert(self.flush(child_idx).i().get_Node_children().map =~= self.i().flush(
            child_domain.key_set(),
...
impl QueryReceipt {
            ...
            self.i().valid(),
    {
        ...
        assert forall|i: int| 0 <= i < i_receipt.lines.len() implies #[trigger]
        i_receipt.lines[i].wf() by {
            self.lines[i].node.i_wf();
        }
        assert forall|i: int| 0 <= i < i_receipt.lines.len() - 1 implies #[trigger]
        i_receipt.child_linked_at(i) by {
...
impl Path {
        ...
        decreases self.depth, 0nat,
    {
        ...
        if 0 < self.subpath().depth {
            ...
            assert forall|i|
                #[trigger]
                self.node.valid_child_index(i) ==> new_children[i as int].wf() by {}
        }
    }
        ...
        decreases self.depth,
    {
        ...
        if 0 < self.depth {
            ...
            self.subpath_commutes_with_i();
            self.node.i_children_lemma();
            assert(self.substitute(replacement).i().get_Node_children().map =~= self.i().substitute(
                replacement.i(),
            ).get_Node_children().map);
        }
    }
...
impl PivotBetree::Label {
    pub open spec(checked) fn i(self) -> PagedBetree::Label {
        match self {
            PivotBetree::Label::Query { end_lsn, key, value } => PagedBetree::Label::Query {
                end_lsn: end_lsn,
                key: key,
                value: value,
            },
            PivotBetree::Label::Put { puts } => PagedBetree::Label::Put { puts: puts },
            PivotBetree::Label::FreezeAs { stamped_betree } => PagedBetree::Label::FreezeAs {
...
impl PivotBetree::State {
    ...
    pub proof fn init_refines(self, stamped_betree: StampedBetree)
        requires
            PivotBetree::State::initialize(self, stamped_betree),
        ensures
            PagedBetree::State::initialize(self.i(), i_stamped_betree(stamped_betree)),
    {
            ...
            PagedBetree::State::next(self.i(), post.i(), lbl.i()),
    {
        ...
        receipt.i_valid();
        assert(PagedBetree::State::next_by(
            self.i(),
            post.i(),
            lbl.i(),
            PagedBetree::Step::query(receipt.i()),
        ));
    }
    ...
    pub proof fn internal_flush_memtable_refines(self, post: Self, lbl: PivotBetree::Label)
        requires
            self.inv(),
            PivotBetree::State::internal_flush_memtable(self, post, lbl),
        ensures
            post.inv(),
            PagedBetree::State::next(self.i(), post.i(), lbl.i()),
    {
        ...
        let equiv_children_node = if self.root.is_Node() {
            self.root
        } else {
            BetreeNode::empty_root(total_domain())
        };
            ...
            PagedBetree::State::next(self.i(), post.i(), lbl.i()),
    {
        ...
        assert(post.i().root.get_Node_children().map =~= PagedBetree_v::constant_child_map(
            self.root.i(),
        ).map);
        assert(PagedBetree::State::next_by(
            self.i(),
    ...
    pub proof fn internal_split_refines(
        ...
        requires
            self.inv(),
            PivotBetree::State::internal_split(self, post, lbl, path, request),
        ensures
            post.inv(),
            PagedBetree::State::next(self.i(), post.i(), lbl.i()),
    {
        ...
        assert(PagedBetree::State::next_by(
            ...
            post.i(),
            lbl.i(),
            PagedBetree::Step::internal_split(path.i(), left_keys, right_keys),
        ));
    }
    ...
    pub proof fn internal_flush_refines(
        ...
        requires
            self.inv(),
            PivotBetree::State::internal_flush(self, post, lbl, path, child_idx),
        ensures
            post.inv(),
            PagedBetree::State::next(self.i(), post.i(), lbl.i()),
    {
