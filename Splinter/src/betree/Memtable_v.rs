...
impl Memtable {
    ...
    pub proof fn apply_puts_end(self, puts: MsgHistory)
        requires
            ...
            puts.can_follow(self.seq_end),
        ensures
            self.apply_puts(puts).seq_end == puts.seq_end,
        decreases puts.len(),
    {
        if 0 < puts.len() {
            let last_lsn = (puts.seq_end - 1) as nat;
    ...
    pub proof fn apply_puts_additive(self, puts1: MsgHistory, puts2: MsgHistory)
        ...
        ensures
            self.apply_puts(puts1).apply_puts(puts2) == self.apply_puts(puts1.concat(puts2)),
    ...
    pub open spec(checked) fn is_empty(self) -> bool {
        self.buffer
            == Buffer::empty()// Note(Jialin): not suited here bc if map is not finite len has no meaning
