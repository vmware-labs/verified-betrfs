...
impl PivotTable {
    ...
    pub open spec(checked) fn wf(self) -> bool {
        ...
        &&& (forall|i: int|
            0 <= i < self.num_ranges() ==> (#[trigger]
            self.pivots[i].is_Elem()))
    }
    ...
    pub open spec(checked) fn can_insert(self, i: int, element: Element) -> bool {
        ...
        &&& (0 < i && i < self.len() ==> Element::lt(self.pivots[i - 1], element) && Element::lt(
            element,
            self.pivots[i],
        ))
    }
    ...
    pub proof fn insert_wf(self, idx: int, element: Element)
        ...
        ensures
            self.insert(idx, element).wf(),
    {
        let result = self.insert(idx, element).pivots;
        Element::lte_transitive_forall();
        assert forall|i: int, j: int| 0 <= i < j < result.len() implies Element::lt(
            result[i],
            result[j],
        ) by {
            if i == idx && j > i + 1 {
    ...
    pub proof fn route_lemma(self, key: Key)
        requires
            ...
            self.bounded_key(key),
        ensures
            0 <= self.route(key) < self.num_ranges(),
    ...
    pub proof fn route_lemma_auto()
        ensures
            forall|pt: PivotTable, key: Key|
                pt.wf() && pt.bounded_key(key) ==> {
                    &&& 0 <= #[trigger]
                    pt.route(key) < pt.num_ranges()
                    &&& Element::lte(pt.pivots[pt.route(key)], to_element(key))
                    &&& Element::lt(to_element(key), pt.pivots[pt.route(key) + 1])
                },
    {
        assert forall|pt: PivotTable, key: Key| pt.wf() && pt.bounded_key(key) implies {
            &&& 0 <= #[trigger]
            pt.route(key) < pt.num_ranges()
            &&& Element::lte(pt.pivots[pt.route(key)], to_element(key))
            &&& Element::lt(to_element(key), pt.pivots[pt.route(key) + 1])
    ...
    pub proof fn route_is_lemma_auto()
        ensures
            forall|pt: PivotTable, key: Key, r: int|
                {
