...
impl BufferSeq {
    ...
    pub proof fn subdisk_implies_same_i(self, small: DiskView, big: DiskView)
        ...
        ensures
            self.valid(big),
            self.i(small) == self.i(big),
    {
        assert forall|i| 0 <= i < self.len() implies self.i(small)[i] == self.i(big)[i] by {
            assert(big.entries.dom().contains(self[i]));  // trigger
        }
        assert(self.i(small) =~= self.i(big));
    }

    pub open spec fn key_in_buffer(
        self,
        dv: DiskView,
        from_idx: int,
        k: Key,
    ...
    pub proof fn query_from_commutes_with_i(self, dv: DiskView, k: Key, start: int)
        requires
            ...
            self.valid(dv),
        ensures
            self.query_from(dv, k, start) == self.i(dv).query_from(k, start),
        decreases self.len() - start,
    {
        if start < self.len() {
