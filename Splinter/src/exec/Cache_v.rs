...
state_machine!{ Cache {
    ...
    // allows for atomically reading and writing a number of slots
    ...
    transition!{ evict(lbl: Label, evicted_slots: Set<Slot>) {
        ...
        update entries = pre.entries.union_prefer_right(updated_entries);
        update status_map = pre.status_map.remove_keys(evicted_slots);
    ...
    init!{ initialize(slots: nat) {
        ...
        init status_map = Map::empty();
        // init total_slots = slots;
    }}
    ...
    pub open spec(checked) fn inv(self) -> bool {
        ...
        &&& forall |slot| #[trigger] self.status_map.contains_key(slot)
            <==> (self.entries.contains_key(slot) && self.entries[slot] is Filled)
    }
