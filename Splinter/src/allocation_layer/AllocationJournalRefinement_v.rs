...
impl AllocationJournal::Step {
    pub open spec fn i(self) -> LikesJournal::Step {
        match self {
            Self::read_for_recovery() => LikesJournal::Step::read_for_recovery(),
            Self::freeze_for_commit(depth) => LikesJournal::Step::freeze_for_commit(depth),
            Self::query_end_lsn() => LikesJournal::Step::query_end_lsn(),
            Self::put(new_journal) => LikesJournal::Step::put(new_journal),
            Self::discard_old(new_journal) => LikesJournal::Step::discard_old(new_journal),
            Self::internal_journal_marshal(
                cut,
                addr,
                new_journal,
            ) => LikesJournal::Step::internal_journal_marshal(cut, addr, new_journal),
            Self::internal_mini_allocator_fill() => LikesJournal::Step::internal_no_op(),
            Self::internal_mini_allocator_prune() => LikesJournal::Step::internal_no_op(),
            Self::internal_no_op() => LikesJournal::Step::internal_no_op(),
...
impl AllocationJournal::Label {
    pub open spec(checked) fn i(self) -> LikesJournal::Label {
        match self {
            ...
            Self::FreezeForCommit { frozen_journal } => LikesJournal::Label::FreezeForCommit {
            ...
            Self::InternalAllocations { allocs, deallocs } => LikesJournal::Label::Internal {  },
        }
    }
}
...
impl DiskView {
    ...
    pub proof fn build_lsn_au_index_page_walk_consistency(self, root: Pointer)
        ...
        ensures
            ...
            forall|lsn|
                self.build_lsn_addr_index(root).contains_key(lsn) ==> #[trigger]
                self.build_lsn_addr_index(root)[lsn].au == self.build_lsn_au_index_page_walk(
                    root,
                )[lsn],
...
impl AllocationJournal::State {
            ...
            ),
    {
        ...
        if frozen_root is Some {
            assert forall|lsn|
                #[trigger]
                i_frozen_index.contains_key(lsn) implies !addrs_past_new_end.contains(
                i_frozen_index[lsn],
            ) by {
                let addr = i_frozen_index[lsn];
                self.tj().build_lsn_addr_index_ensures();
                assert(self.tj().disk_view.index_keys_map_to_valid_entries(
                    self.i().lsn_addr_index,
                ));
                self.tj().disk_view.instantiate_index_keys_map_to_valid_entries(
                    self.i().lsn_addr_index,
                    lsn,
                );
                ...
                if addrs_past_new_end.contains(addr) {
                    ...
                    assert(self.tj().disk_view.addr_supports_lsn(
                        frozen_root.unwrap(),
                        last_frozen_lsn,
                    ));
                    assert(self.tj().disk_view.addr_supports_lsn(addr, lsn));
    ...
    pub proof fn discard_old_refines(
        ...
        post: Self,
        lbl: AllocationJournal::Label,
        new_journal: LinkedJournal::State,
    )
        requires
            self.inv(),
            post.inv(),
            ...
            ),
    {
        ...
        let keep_addrs = Set::new(
            |addr: Address| addr.wf() && post.lsn_au_index.values().contains(addr.au),
        );
        let lsn_addr_index_post = LikesJournal_v::lsn_addr_index_discard_up_to(
            self.i().lsn_addr_index,
            start_lsn,
        );
        ...
        post.tj().disk_view.sub_disk_with_newer_lsn_repr_index(
            self.tj().disk_view,
            post.tj().freshest_rec,
        );
    ...
    pub proof fn internal_journal_marshal_refines(
        self,
        post: Self,
        lbl: AllocationJournal::Label,
        cut: LSN,
        ...
        requires
            self.inv(),
            post.inv(),
            Self::internal_journal_marshal(self, post, lbl, cut, addr, new_journal),
        ensures
            LikesJournal::State::next_by(
                self.i(),
                post.i(),
    ...
    pub proof fn init_refines(self, journal: LinkedJournal::State, image: JournalImage)
        requires
            AllocationJournal::State::initialize(self, journal, image),
        ensures
            LikesJournal::State::initialize(self.i(), image.tj),
    {
    }
}
