...
impl Ephemeral {
    pub open spec fn i(self) -> AbstractCrashAwareJournal_v::Ephemeral {
        if self is Unknown {
            ...
            AbstractCrashAwareJournal_v::Ephemeral::Known { v: self.get_Known_v().i_abstract() }
        }
    }
}
...
impl AllocationCrashAwareJournal::Label {
    pub open spec fn i(self) -> CrashTolerantJournal::Label {
        match self {
            Self::LoadEphemeralFromPersistent => CrashTolerantJournal::Label::LoadEphemeralFromPersistentLabel,
            Self::ReadForRecovery { records } => CrashTolerantJournal::Label::ReadForRecoveryLabel {
                records,
            },
            Self::QueryEndLsn { end_lsn } => CrashTolerantJournal::Label::QueryEndLsnLabel {
                end_lsn,
            },
            Self::Put { records } => CrashTolerantJournal::Label::PutLabel { records },
            Self::Internal { allocs, deallocs } => CrashTolerantJournal::Label::InternalLabel,
            Self::QueryLsnPersistence {
                sync_lsn,
...
impl AllocationCrashAwareJournal::State {
    pub open spec fn i(self) -> CrashTolerantJournal::State {
        let i_in_flight = if self.inflight is None {
            None
        } else {
            Some(self.inflight.unwrap().i())
        };
    ...
    pub proof fn load_ephemeral_from_persistent_refines(
        ...
        ensures
            CrashTolerantJournal::State::next_by(
                ...
                post.i(),
                lbl.i(),
                CrashTolerantJournal::Step::load_ephemeral_from_persistent(
    ...
    pub proof fn read_for_recovery_refines(
        ...
        ensures
            CrashTolerantJournal::State::next_by(
                ...
                post.i(),
                lbl.i(),
                CrashTolerantJournal::Step::read_for_recovery(),
            ),
    {
        reveal(CrashTolerantJournal::State::next_by);
        let aj = self.ephemeral.get_Known_v();
        let alloc_lbl = AllocationJournal::Label::ReadForRecovery {
    ...
    pub proof fn query_end_lsn_refines(self, post: Self, lbl: AllocationCrashAwareJournal::Label)
        ...
        ensures
            CrashTolerantJournal::State::next_by(
                ...
                post.i(),
                lbl.i(),
                CrashTolerantJournal::Step::query_end_lsn(),
            ),
    {
        reveal(CrashTolerantJournal::State::next_by);
        let aj = self.ephemeral.get_Known_v();
        let alloc_lbl = AllocationJournal::Label::QueryEndLsn {
        ...
        match step {
            ...
            AllocationCrashAwareJournal::Step::query_lsn_persistence() => {
                assert(CrashTolerantJournal::State::next_by(
                    ...
                    post.i(),
                    lbl.i(),
                    CrashTolerantJournal::Step::query_lsn_persistence(),
                ));  // witness
            },
