...
impl JournalRecord {
    ...
    fn i(self, boundary_lsn: LSN) -> MsgHistory
        recommends
            self.wf(),
        decreases self,
    {
    ...
    pub open spec   /*XXX (checked)*/
    fn i_opt(ojr: Option<Self>, boundary_lsn: LSN) -> MsgHistory
        recommends
    ...
    {
        assert forall|selff: Self, boundary_lsn: LSN| selff.valid(boundary_lsn) implies ({
            let out = selff.i(boundary_lsn);
            &&& out.wf()
            &&& out.seq_start == boundary_lsn
            &&& out.seq_end === selff.message_seq.seq_end
    ...
    pub proof fn cant_crop(self, bdy: LSN, depth: nat)
        requires
            ...
            self.crop_head_records(bdy, (depth - 1) as nat).unwrap().message_seq.can_discard_to(
                bdy,
            ),
        ensures
    ...
    pub proof fn crop_head_records_chaining(self, bdy: LSN, depth: nat)
        ...
        ensures
            self.crop_head_records(bdy, (depth - 1) as nat).unwrap().cropped_prior(bdy)
        ...
        decreases depth,
    {
        ...
        if 1 < depth {
            ...
            assert(self.crop_head_records(bdy, depth) == Self::opt_rec_crop_head_records(
    ...
    pub proof fn can_cut_more(ojr: Option<JournalRecord>, bdy: LSN, depth: nat)
        ...
        ensures
            ...
            Self::opt_rec_can_crop_head_records(
                Self::opt_rec_crop_head_records(ojr, bdy, (depth - 1) as nat),
                bdy,
                1,
            ),
        decreases depth,
    ...
    pub proof fn crop_equivalence(ojr: Option<JournalRecord>, bdy: LSN, depth: nat)
        ...
        ensures
            ...
            Self::opt_rec_crop_head_records(ojr, bdy, depth) == Self::opt_rec_crop_head_records(
                Self::opt_rec_crop_head_records(ojr, bdy, (depth - 1) as nat),
                bdy,
                1,
            ),
    ...
    pub proof fn discard_old_maintains_subseq(
        ...
        ensures
            ...
            Self::i_opt(ojr, old_bdy).includes_subseq(
                Self::i_opt(Self::discard_old_journal_rec(ojr, new_bdy), new_bdy),
            ),
        decreases ojr,
    {
    ...
    pub proof fn crop_head_maintains_subseq(ojr: Option<JournalRecord>, bdy: LSN, depth: nat)
        requires
            ...
            Self::opt_rec_can_crop_head_records(ojr, bdy, depth),
        ensures
            Self::i_opt(ojr, bdy).includes_subseq(
                Self::i_opt(Self::opt_rec_crop_head_records(ojr, bdy, depth), bdy),
            ),
        decreases depth,
    {
        if depth > 0 {
            ...
            assert(small.unwrap().crop_head_records(bdy, 1) == Self::opt_rec_crop_head_records(
                small.unwrap().cropped_prior(bdy),
                bdy,
                0,
            ));  // trigger, newly required
    ...
    pub proof fn discard_old_defn_interprets(ojr: Option<JournalRecord>, old_lsn: LSN, new_lsn: LSN)
        ...
        ensures
            Self::i_opt(Self::discard_old_journal_rec(ojr, new_lsn), new_lsn) == Self::i_opt(
                ojr,
                old_lsn,
            ).discard_old(new_lsn),
        decreases ojr,
...
impl TruncatedJournal {
            ...
            self.i().includes_subseq(frozen.i()),
    {
        ...
        if ctj.freshest_rec.is_Some() && frozen.boundary_lsn
            < ctj.freshest_rec.unwrap().message_seq.seq_end {
            self.freshest_rec.unwrap().crop_head_records_lemma(self.boundary_lsn, depth);  // newly required
            JournalRecord::discard_old_maintains_subseq(
                ctj.freshest_rec,
                ctj.boundary_lsn,
                frozen.boundary_lsn,
            );
...
impl PagedJournal::Label {
    ...
    {
        match self {
            PagedJournal::Label::ReadForRecovery {
                messages,
            ...
            PagedJournal::Label::QueryEndLsn {
                end_lsn,
            } => AbstractJournal::Label::QueryEndLsnLabel { end_lsn },
            PagedJournal::Label::Put { messages } => AbstractJournal::Label::PutLabel { messages },
            PagedJournal::Label::DiscardOld {
                start_lsn,
                require_end,
            } => AbstractJournal::Label::DiscardOldLabel { start_lsn, require_end },
            PagedJournal::Label::Internal {  } => AbstractJournal::Label::InternalLabel {  },
        }
    }
}
...
impl PagedJournal::State {
            ...
            AbstractJournal::State::next(self.i(), post.i(), lbl.i()),
    {
        ...
        self.truncated_journal.tj_freeze_for_commit(
            ...
            AbstractJournal::State::next(self.i(), post.i(), lbl.i()),
    {
        ...
        match step {
            ...
            _ => {
                assert(false);
            }  // dummy_to_use_type_params boilerplate
            ,
        }
    }
    ...
    pub proof fn init_refines(self, truncated_journal: TruncatedJournal)
        requires
            PagedJournal::State::initialize(self, truncated_journal),
        ensures
            AbstractJournal::State::initialize(self.i(), self.i().journal),
    {
        JournalRecord::i_lemma_forall();
    }
}
