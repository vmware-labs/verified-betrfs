    ...
    ensures CSL.Next(Ik(k), I(k, s), I(k, s'))
  {
    ...
    match mstep {
      case QueryStep(idx: DLSI.Index, result: Element) => {
        assert CSL.LS.NextStep(ik, iis.ephemeral, iis'.ephemeral, CSL.LS.QueryStep(idx, result)); // witness
        assert CSL.NextStep(Ik(k), I(k, s), I(k, s'), CSL.EphemeralMoveStep); // witness
      }
      case FetchSuperblockStep(length: int) => {
        assert CSL.LS.NextStep(ik, iis.ephemeral, iis'.ephemeral, CSL.LS.StutterStep); // witness
        assert CSL.NextStep(Ik(k), I(k, s), I(k, s'), CSL.EphemeralMoveStep); // witness
      }
      ...
      case AppendStep(element: Element) => {
        assert CSL.LS.NextStep(ik, iis.ephemeral, iis'.ephemeral, CSL.LS.AppendStep(element)); // witness
        assert CSL.NextStep(Ik(k), I(k, s), I(k, s'), CSL.EphemeralMoveStep); // witness
      }
