...
module CacheIOImpl {
  ...
  requires ticket == DiskWriteTicket(disk_idx * 4096, contents.s)
  {
    var idx;
    linear var access;
    idx, access := get_free_io_slot(c);
    linear var IOSlotAccess(aiocb, info) := access;

    ptr_write(c.io_slots[idx].aiocb_ptr,
        inout aiocb,
        aiocb_constructor(disk_idx * 4096, c.data[cache_idx], 4096));

    ptr_write(c.io_slots[idx].info_ptr,
        inout info,
    ...
    old_v, new_v, old_g := async_write(
        c.disk_interface,
        c.io_slots[idx].aiocb_ptr,
        aiocb,
        contents,
        ticket);
    ///////////////////////////////////////////////////////////////////
    linear var DIGhost(writeMap, readMap) := old_g;
    writeMap := LinearMaps.add(writeMap, c.io_slots[idx].aiocb_ptr,
        WritebackGhostStateWithSlot(info, g));
    linear var new_g := DIGhost(writeMap, readMap);
    ///////////////////////////////////////////////////////////////////
    disk_interface_op_cleanup(c.disk_interface, new_v, new_g);
    ///////////////////////////////////////////////////////////////////
  }
