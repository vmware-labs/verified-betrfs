...
state_machine_line = seq(whitespace, string('state').mark().tag('keyword'), whitespace, string('machine').mark().tag('keyword'), whitespace, \
        ...
        string('s').mark().tag('keyword'), whitespace, string('(').mark().tag('syntax'), params, string(')').mark().tag('syntax'), whitespace).mark().tag('tla.statemachine')
...
def walk(contents):
    ...
    for element in contents:
        if isinstance(element, list):
            (element_has_state_machine, element_state_machine_indent, element_collected_steps) = walk(element)
            ...
            if name == "tla.statemachine":
                ...
                step_gen_code = 'k: Constants, s: Variables, s\': Variables' + (', ' if step_params_code.strip() != '' else '') + step_params_code
                inner.insert(6, ('tladfy', (step_params_start, step_gen_code, step_params_end)))

                collected_steps.append((inner[3][1][1], step_params_code))

            if isinstance(inner, list):
                (inner_has_state_machine, inner_state_machine_indent, inner_collected_steps) = walk(inner)
                if inner_has_state_machine:
                    ...
                    nextstep = indent_str + "predicate NextStep(k: Constants, s: Variables, s': Variables, step: Step)\n" + \
                            ...
                            "\n".join(["{}    case {}Step({}) => {}(k, s, s'{}{})".format(indent_str, ident, pnames, ident, ', ' if pnames.strip() != "" else '', pnames) for ((ident, params), pnames) in nextstep_names]) + "\n" + \
                            indent_str + "  }\n" + \
                            indent_str + "}\n"

                    predicate_next = indent_str + "predicate Next(k: Constants, s: Variables, s': Variables)" + "\n" + \
                            indent_str + "{" + "\n" + \
                            indent_str + "  exists step :: NextStep(k, s, s', step)" + "\n" + \
                            indent_str + "}" + "\n"
    ...
    return (has_state_machine, state_machine_indent, collected_steps)
