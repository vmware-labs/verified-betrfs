...
include "../disciplined/common/MultisetLemmas.i.dfy"
include "../disciplined/common/MonoidLib.i.dfy"
...
module SummaryMonoid refines MonoidMap {
  ...
  datatype QueryRes =
    | QueryFound(rid: RequestId, key: Key, value: Option<Value>)
    | QueryUnknown(rid: RequestId, key: Key)

  datatype Summary = Summary(
    ops: map<Key, Option<Value>>,
    stubs: multiset<HT.Response>,
    ...
    match info {
      ...
      case Info(Empty(), Inserting(rid, ins_kv, init_key)) =>
        Summary(
          ...
          multiset{HT.Response(rid, MapIfc.InsertOutput(true))},
          multiset{},
          multiset{}
        )
...
module Interpretation {
  ...
  function interp_with_stubs(s: M) : S.Summary
  requires Inv(s)
  ...
  function to_query_stub(q: S.QueryRes) : HT.Response {
    match q {
      case QueryFound(rid, key, Some(value)) =>
          HT.Response(rid, MapIfc.QueryOutput(Found(value)))
      case QueryFound(rid, key, None()) =>
          HT.Response(rid, MapIfc.QueryOutput(NotFound))
      case QueryUnknown(rid, key) =>
          HT.Response(rid, MapIfc.QueryOutput(NotFound))
    }
  }

  function apply_to_query_stub(m: multiset<S.QueryRes>) : multiset<HT.Response> {
    Multisets.Apply(to_query_stub, m)
  }

  function to_remove_stub(q: S.QueryRes) : HT.Response {
    match q {
      case QueryFound(rid, key, Some(value)) =>
          HT.Response(rid, MapIfc.RemoveOutput(true))
      case QueryFound(rid, key, None()) =>
          HT.Response(rid, MapIfc.RemoveOutput(false))
      case QueryUnknown(rid, key) =>
          HT.Response(rid, MapIfc.RemoveOutput(false))
    }
  }

  function apply_to_remove_stub(m: multiset<S.QueryRes>) : multiset<HT.Response> {
  ...
  lemma InsertSkip_PreservesInterp(s: M, s': M, pos: nat)
  requires Inv(s)
  ...
  lemma InsertSwap_PreservesInterp(s: M, s': M, pos: nat)
  requires Inv(s)
  ...
  lemma InsertDone_PreservesInterp(s: M, s': M, pos: nat)
  requires Inv(s)
  requires HT.InsertDone(s, s', pos)
  ensures Inv(s')
  ensures interp(s.table).ops == interp(s'.table).ops
  ensures interp(s.table).stubs == interp(s'.table).stubs + multiset{HT.Response(s.table[pos].value.state.rid, MapIfc.InsertOutput(true))}
  ...
  ensures interp(s.table).removes == interp(s'.table).removes
  {
    ...
    var x := S.Summary(map[],
        multiset{HT.Response(s.table[pos].value.state.rid, MapIfc.InsertOutput(true))},
        multiset{}, multiset{});
  ...
  lemma InsertUpdate_PreservesInterp(s: M, s': M, pos: nat)
  requires Inv(s)
  ...
  lemma QuerySkip_PreservesInterp(s: M, s': M, pos: nat)
  requires Inv(s)
  ...
  lemma QueryDone_PreservesInterp(s: M, s': M, pos: nat)
  requires Inv(s)
  ...
  lemma QueryNotFound_PreservesInterp(s: M, s': M, pos: nat)
  requires Inv(s)
  ...
  lemma RemoveSkip_PreservesInterp(s: M, s': M, pos: nat)
  requires Inv(s)
  ...
  lemma RemoveFoundIt_PreservesInterp(s: M, s': M, pos: nat)
  requires Inv(s)
  ...
  lemma RemoveTidy_PreservesInterp(s: M, s': M, pos: nat)
  requires Inv(s)
  ...
  lemma RemoveDone_PreservesInterp(s: M, s': M, pos: nat)
  requires Inv(s)
  ...
  lemma RemoveNotFound_PreservesInterp(s: M, s': M, pos: nat)
  requires Inv(s)
  ...
  ensures apply_to_remove_stub(interp(s.table).removes)
       == apply_to_remove_stub(interp(s'.table).removes)
        + multiset{HT.Response(s.table[pos].value.state.rid, MapIfc.RemoveOutput(false))};

  {
    ...
    calc {
          ...
          s.table[pos].value.state.rid, s.table[pos].value.state.key)});
      {
        ...
        assert to_remove_stub(S.QueryUnknown(s.table[pos].value.state.rid, s.table[pos].value.state.key)) == HT.Response(s.table[pos].value.state.rid, MapIfc.RemoveOutput(false));
  ...
  lemma ProcessInsertTicket_ChangesInterp(s: M, s': M, ticket: HT.Request)
  requires Inv(s)
  ...
  ensures interp(s'.table).stubs == interp(s.table).stubs
      + multiset{HT.Response(ticket.rid, MapIfc.InsertOutput(true))}
  ensures interp(s'.table).queries == interp(s.table).queries
  ensures interp(s'.table).removes == interp(s.table).removes
  {
  ...
  lemma ProcessQueryTicket_ChangesInterp(s: M, s': M, ticket: HT.Request)
  requires Inv(s)
  ...
  ensures ticket.input.key !in map_remove_nones(interp(s.table).ops) ==>
        apply_to_query_stub(interp(s'.table).queries)
         == apply_to_query_stub(interp(s.table).queries)
          + multiset{HT.Response(ticket.rid, MapIfc.QueryOutput(NotFound))}
  ensures ticket.input.key in map_remove_nones(interp(s.table).ops) ==>
        apply_to_query_stub(interp(s'.table).queries)
         == apply_to_query_stub(interp(s.table).queries)
          + multiset{HT.Response(ticket.rid, MapIfc.QueryOutput(Found(
    ...
    if ticket.input.key !in map_remove_nones(interp(s.table).ops) {
      calc {
          ...
          + apply_to_query_stub(interp(s.table).queries);
        {
          ...
          //    interp(s.table).ops))
          //  == HT.Response(ticket.rid, MapIfc.QueryOutput(NotFound));
  ...
  lemma ProcessRemoveTicket_ChangesInterp(s: M, s': M, ticket: HT.Request)
  requires Inv(s)
  ...
  ensures ticket.input.key !in map_remove_nones(interp(s.table).ops) ==>
        apply_to_remove_stub(interp(s'.table).removes)
         == apply_to_remove_stub(interp(s.table).removes)
          + multiset{HT.Response(ticket.rid, MapIfc.RemoveOutput(false))}
  ensures ticket.input.key in map_remove_nones(interp(s.table).ops) ==>
        apply_to_remove_stub(interp(s'.table).removes)
         == apply_to_remove_stub(interp(s.table).removes)
          + multiset{HT.Response(ticket.rid, MapIfc.RemoveOutput(true))}
  {
