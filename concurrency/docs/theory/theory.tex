...
\begin{align*}
  ...
  &\inlinehoarel{\active(\kappa) * \borrow{\kappa}{\dboxed{ht.entries}^\gamma}
      * \dboxed{k \hashto v_0}^\gamma \land KeyNotInRange(ht.entries, k, i)}\\
  &\inlinehoarer{~~~~
      * \active(\kappa_0) * \borrowed\left(\kappa_0, \SharedGuard\left( (i \entryto c, ht.entries[i] \pointsto c) \right) \right)
      * \borrow{\kappa_0}{\SharedGuard\left( (i \entryto c, ht.entries[i] \pointsto c) \right) }}\\
  &\langlet r = (\langmatch~ !ht.table[i] ~\langwith\\
  &~~~~ |~ \None \Rightarrow \None\\
  &~~~~ |~ \Some~(k_0,v_0) \Rightarrow
      \langif~ k = k_0 ~\langthen~ Some(v_0) ~\langelse~ \QueryIter(ht,k,i+1)\\
