...
module MkfsImpl {
  ...
  ensures ADM.BlocksDontIntersect(diskContents)
  {
    ...
    assert SSM.IIndirectionTable(SSI.IIndirectionTable(sectorIndirectionTable)) == IndirectionTable(
      map[0 := nodeLoc],
      map[0 := []]
    );

    assert BC.WFCompleteIndirectionTable(SSM.IIndirectionTable(SSI.IIndirectionTable(sectorIndirectionTable)));
