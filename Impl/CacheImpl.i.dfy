...
module CacheImpl {
  ...
  linear datatype LMutCache = LMutCache(linear cache: LinearHashMap<Node>) {
    ...
    {
      && LCMM.Inv(cache)
      && (forall ref | ref in cache.contents :: 
        cache.contents[ref].Inv() && BT.WFNode(cache.contents[ref].I()))
    }
    ...
    requires node.Inv() && BT.WFNode(node.I())
    ...
    requires |old_self.I()| <= 0x10000
    requires newNode.Inv() && BT.WFNode(newNode.I())

    ensures self.Inv()
    ensures oldNode.Inv() && BT.WFNode(oldNode.I())
    ...
    requires old_self.ptr(ref).Some?
    ensures self.Inv()
    ensures node.Inv() && BT.WFNode(node.I())
