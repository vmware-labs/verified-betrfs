...
module MarshallingImpl {
  ...
  ensures s.lSome? ==> 
    (
      && s.value.Inv()
      && BT.WFNode(s.value.I())
      && s.value.I() == Marshalling.valToNode(v).value
    )
  {
  ...
  method ValToSector(v: V) returns (linear s : lOption<Sector>)
  requires ValidVal(v)
  ...
  ensures data != null ==>
    Marshalling.parseCheckedSector(data[..]).value == SSI.ISector(sector)
