...
module BookkeepingModel { 
  ...
  predicate writeWithNode(s: ImplVariables, ref: BT.G.Reference, node: Node, s': ImplVariables)
  requires s.W()
  requires s.WriteAllocConditions()
  requires s.ChildrenConditions(node.children)
  ...
  lemma writeCorrect(s: ImplVariables, ref: BT.G.Reference, node: Node, s': ImplVariables)
  requires s.Ready?
  requires s.WFBCVars()
  requires s.WriteAllocConditions()
  ...
  requires s.frozenIndirectionTable.lSome? && ref in s.frozenIndirectionTable.value.graph ==> ref in s.frozenIndirectionTable.value.locs
  requires |s.ephemeralIndirectionTable.graph| < IT.MaxSize()
  requires writeWithNode(s, ref, node, s')
