...
module CRC32_C_Array_Impl {
  ...
  returns (res: uint32)
  requires 0 <= idx0
  requires idx0 as int + len0 as int <= data_array.Length
  requires data_array.Length < 0x1_0000_0000
  ensures advances_bytes(data_array[..], idx0 as int, prev, idx0 as int + len0 as int, res)
  {
    ghost var data := data_array[..];
    ...
    {
      var x := intrinsic_mm_crc32_u8(crcA as uint32, data_array[pA]);
    ...
    invariant advances_bytes(data, idx0 as int, prev, pA as int, crcA as uint32)
    {
      ...
      {
        /////// A
        var a := Unpack_LittleEndian_Uint64_From_Array(data_array, pA - (8*z) as uint64);
        crcA := intrinsic_mm_crc32_u64(crcA, a);

        /////// B
        var b := Unpack_LittleEndian_Uint64_From_Array(data_array, pB - (8*z) as uint64);
        crcB := intrinsic_mm_crc32_u64(crcB, b);

        /////// C
        var c := Unpack_LittleEndian_Uint64_From_Array(data_array, pC - (8*z) as uint64);
      ...
      var a := Unpack_LittleEndian_Uint64_From_Array(data_array, pA - 8);
      ...
      var b := Unpack_LittleEndian_Uint64_From_Array(data_array, pB - 8);
      ...
      var c := Unpack_LittleEndian_Uint64_From_Array(data_array, pC - 8);
    ...
    invariant advances_bytes(data, idx0 as int, prev, pA as int, crcA as uint32)
    {
      var a := Unpack_LittleEndian_Uint64_From_Array(data_array, pA);
      var new_crcA := intrinsic_mm_crc32_u64(crcA, a);
  ...
  method compute_crc32c_padded(data: array<byte>, i: uint32, len: uint32)
  returns (checksum: seq<byte>)
  requires i as int + len as int <= data.Length < 0x1_0000_0000
  ensures checksum == A.crc32_c_padded(data[i..i+len])
  {
    var s := compute_crc32_main(data, i, len, 0xffffffff);
    var t := bitxor32(s, 0xffffffff);

    var ar := NativeArrays.newArrayFill(32, 0);
    Pack_LittleEndian_Uint32_into_Array(t, ar, 0);

    final_comp(data[..], s, i as int, i as int + len as int, ar[0..4]);
    assert data[..][i..i+len] == data[i..i+len];

    calc {
      ar[..];
      ar[0..4] + ar[4..32];
      {
        assert ar[0..4] == crc32_c(data[i..i+len]);
        assert ar[4..32] == seq(28, i => 0);
      }
      crc32_c(data[i..i+len]) + seq(28, i => 0);
    }
